...
import { Tarea } from "../Utilidades/tipos";
import { useColoresDelTema } from "../Contexto/ContextoDeTema";
import { formatearFecha, formatearDuracion, nombreVisible } from "../Utilidades/formato";
...
export function PantallaTareas({ navigation }: any) {
  ...
  async function toggleDone(task: Tarea) {
    if (!trip) return;
    if (task.status === "DONE") {
      await api.descompletarTarea(trip.id, task.id);
      reload();
      return;
    }
    if (task.timeTracked) {
  ...
  async function finishTask(task: Tarea, durationSeconds: number | undefined) {
    if (!trip) return;
    setConfirmingComplete(true);
    try {
      const result = await api.completarTarea(trip.id, task.id, durationSeconds != null ? { durationSeconds } : undefined);
      const durationText = result.durationSeconds != null ? ` (te llevó ${formatearDuracion(result.durationSeconds)})` : "";
  ...
  function confirmDelete(task: Tarea) {
    Alert.alert("Eliminar tarea", `¿Eliminar "${task.title}"?`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.eliminarTarea(trip!.id, task.id);
          reload();
        },
      },
    ]);
          ...
          <View style={styles.card}>
            ...
            >
              ...
              <View style={styles.subRow}>
                <Text style={styles.cardSub}>Asignada a: {item.assignedTo ? nombreVisible(item.assignedTo) : "Sin asignar"}</Text>
                ...
                {item.timeTracked && item.startDate && item.status === "PENDING" && (
                  <View style={styles.timerBadge}>
                    ...
                    <Cronometro startDate={item.startDate} style={styles.timerText} />
                  </View>
                )}
              </View>
            </TouchableOpacity>
            ...
            {item.status === "DONE" && item.completions?.[0] && (
              <Text style={styles.dueDate}>{formatearFecha(item.completions[0].completedAt)}</Text>
            )}

...
export function PantallaNuevaTarea({ navigation, route }: any) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isEditing) {
        await api.actualizarTarea(trip.id, task!.id, { ...payload, assignedToId: assignmentType === "MANUAL" ? assignedToId : undefined });
      } else {
        await api.crearTarea(trip.id, {
          ...payload,
          assignmentType,
          ...
          <View>
            ...
            <View style={styles.chipsRow}>
              {trip.members.map((m) => (
                ...
                >
                  <Text style={[styles.chipText, assignedToId === m.userId && styles.chipTextActive]}>{nombreVisible(m.user)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>

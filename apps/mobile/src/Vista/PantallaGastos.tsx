...
import { Gasto } from "../Utilidades/tipos";
import { useColoresDelTema } from "../Contexto/ContextoDeTema";
import { plata, formatearFecha, nombreVisible, resumenDePagadores } from "../Utilidades/formato";
...
export function PantallaGastos({ navigation }: any) {
  ...
  function confirmDelete(expense: Gasto) {
    Alert.alert("Eliminar gasto", `¿Eliminar "${expense.description}"? Los saldos se recalculan automáticamente.`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.eliminarGasto(trip!.id, expense.id);
          reload();
        },
      },
    ]);
                ...
                <View style={styles.paidByRow}>
                  {item.payers[0] && (
                    <Avatar
                      ...
                      name={nombreVisible(item.payers[0].user)}
                      color={item.payers[0].user.avatarColor}
                      size={16}
                    />
                  )}
                  <Text style={styles.cardSub}> Pagó: {resumenDePagadores(item.payers)}</Text>
                </View>
              </View>
              <View style={{ alignItems: "flex-end" }}>
                <Text style={styles.amount}>{plata(item.amount)}</Text>
                <Text style={styles.cardSub}>{formatearFecha(item.expenseDate)}</Text>
              </View>
            </TouchableOpacity>

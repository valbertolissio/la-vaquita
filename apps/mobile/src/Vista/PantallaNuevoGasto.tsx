...
import { Categoria, Gasto } from "../Utilidades/tipos";
...
export function PantallaNuevoGasto({ navigation, route }: any) {
  const { colors, mode } = useColoresDelTema();
  const { trip } = useProyecto();
  const expense: Gasto | undefined = route?.params?.expense;
  ...
  async function pickReceiptFrom(source: "camera" | "library") {
    ...
    try {
      ...
      const scan = await api.escanearComprobante(trip!.id, uri);
      ...
      if (scan.items && scan.items.length > 0) {
        ...
        setScanItems([{ description: scan.merchant ?? "", amount: String(scan.amount), checked: true }]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isEditing) {
        await api.actualizarGasto(trip.id, expense!.id, payload);
      } else {
        await api.crearGasto(trip.id, payload);
      }
      navigation.goBack();
    } catch (e: any) {
      Alert.alert("No se pudo guardar", e.message);
  ...
  const styles = StyleSheet.create({
    ...
    addItemText: { color: colors.greenDark, fontWeight: "600", fontSize: 13 },
        ...
        {tab === "ocr" ? (
          scanItems.length > 0 ? (
            <>
              ...
              <TouchableOpacity style={styles.addItemButton} onPress={agregarItem}>
                <Feather name="plus" size={14} color={colors.greenDark} />
              ...
              <View>
                ...
                <View style={styles.chipsRow}>
                  {trip.members.map((m) => (
                    ...
                    >
                      <Text style={[styles.chipText, ocrPaidById === m.userId && styles.chipTextActive]}>{nombreVisible(m.user)}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              ...
              <View>
                ...
                <View style={styles.chipsRow}>
                  {trip.members.map((m) => (
                    ...
                    >
                      <Text style={[styles.chipText, splitBetween.includes(m.userId) && styles.chipTextActive]}>{nombreVisible(m.user)}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              ...
              <View style={styles.chipsRow}>
                {trip.members.map((m) => (
                  <TouchableOpacity key={m.userId} style={[styles.chip, payerIds.includes(m.userId) && styles.chipActive]} onPress={() => togglePayer(m.userId)}>
                    <Text style={[styles.chipText, payerIds.includes(m.userId) && styles.chipTextActive]}>{nombreVisible(m.user)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              {payerIds.length > 1 && (
                  ...
                  {payerIds.map((id) => {
                    ...
                    return (
                      <View key={id} style={styles.payerAmountRow}>
                        <Text style={styles.payerAmountName}>{nombreVisible(member.user)}</Text>
                        <TextInput

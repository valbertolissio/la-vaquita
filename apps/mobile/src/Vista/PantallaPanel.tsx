...
import { ResumenDelProyecto } from "../Utilidades/tipos";
...
export function PantallaPanel({ navigation }: any) {
  ...
  const reload = useCallback(() => {
    if (trip) api.obtenerResumen(trip.id).then(setSummary);
  }, [trip]);
  ...
  async function markSettlementPaid(index: number) {
    ...
    try {
      await api.crearPago(trip.id, { fromUserId: s.fromUserId, toUserId: s.toUserId, amount: s.amount });
      reload();
    } catch (e: any) {
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.heroBalanceValue, { color: summary.myBalance >= 0 ? colors.greenDark : colors.danger }]}>
                {summary.myBalance >= 0 ? "+ " : "- "}
                {plata(Math.abs(summary.myBalance))}
              </Text>
                    ...
                    return (
                      <View key={p.userId}>
                        <View style={styles.timeRow}>
                          ...
                          <Text style={styles.rowAmount}>{formatearDuracion(p.totalSeconds)}</Text>
                        </View>
              ...
              <View style={styles.card}>
                ...
                <View style={styles.rowBetween}>
                  <View>
                    ...
                    <Text style={styles.cardSub}>Pagó: {resumenDePagadores(summary.recentExpenses[0].payers)}</Text>
                  </View>
                  <Text style={styles.rowAmount}>{plata(summary.recentExpenses[0].amount)}</Text>
                </View>
              </View>
            )}
                ...
                <View style={styles.rowBetween}>
                  <View>
                    ...
                    <Text style={styles.cardSub}>
                      Asignada a: {summary.pendingTasks[0].assignedTo ? nombreVisible(summary.pendingTasks[0].assignedTo) : "Sin asignar"}
                    </Text>
                  </View>
                </View>
              </View>

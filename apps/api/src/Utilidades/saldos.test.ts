...
import { SaldoDeParticipante, simplificarDeudas } from "./saldos";

function balance(userId: string, balanceAmount: number): SaldoDeParticipante {
...
describe("simplifyDebts", () => {
  it("suggests nothing when everyone is already even", () => {
    expect(simplificarDeudas([balance("a", 0), balance("b", 0)])).toEqual([]);
  });

  it("settles a simple two-person debt directly", () => {
    const settlements = simplificarDeudas([balance("a", -100), balance("b", 100)]);
  ...
  it("ignores balances within the rounding threshold (a few cents)", () => {
    expect(simplificarDeudas([balance("a", -0.005), balance("b", 0.005)])).toEqual([]);
  });

  it("splits one debtor's debt across multiple creditors", () => {
    const settlements = simplificarDeudas([balance("a", -150), balance("b", 100), balance("c", 50)]);
  ...
  it("pays off one creditor using multiple debtors", () => {
    const settlements = simplificarDeudas([balance("a", -60), balance("b", -40), balance("c", 100)]);
    expect(settlements).toHaveLength(2);

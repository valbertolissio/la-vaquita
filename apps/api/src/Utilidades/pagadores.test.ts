...
import { validarSumaDePagadores } from "./pagadores";

describe("validatePayersSum", () => {
  it("acepta un solo pagador que puso todo", () => {
    expect(validarSumaDePagadores(1000, [{ userId: "a", amount: 1000 }])).toBeNull();
  });
  ...
  it("acepta varios pagadores cuando la suma da el total", () => {
    ...
    expect(validarSumaDePagadores(10000, payers)).toBeNull();
  });
  ...
  it("rechaza cuando lo que pusieron no llega al total", () => {
    ...
    expect(validarSumaDePagadores(10000, payers)).toMatch(/no coincide/i);
  });

  it("rechaza cuando lo que pusieron se pasa del total", () => {
    expect(validarSumaDePagadores(100, [{ userId: "a", amount: 150 }])).toMatch(/no coincide/i);
  });
  ...
  it("tolera la diferencia de un centavo que deja el redondeo", () => {
    ...
    expect(validarSumaDePagadores(100, payers)).toBeNull();
  });
  ...
  it("no se confunde con la aritmética de punto flotante (0.1 + 0.2)", () => {
    ...
    expect(validarSumaDePagadores(0.3, payers)).toBeNull();
  });

  it("rechaza una lista vacía de pagadores contra un gasto con monto", () => {
    expect(validarSumaDePagadores(500, [])).toMatch(/no coincide/i);
  });
});

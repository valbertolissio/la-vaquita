...
export function ListaDeProyectos() {
  ...
  useEffect(() => {
    api.listarProyectos().then(setTrips);
  }, []);

  async function createTrip(e: FormEvent) {
    e.preventDefault();
    const trip = await api.crearProyecto({ name, startDate, endDate });
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Hola, {user ? nombreVisible(user) : ""}</p>
            </div>
          </div>

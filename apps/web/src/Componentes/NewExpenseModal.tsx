...
import { Category, Expense, Trip, TripMember } from "../Utilidades/types";
...
function CategoryPicker({
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const created = await api.createCategory(tripId, { name: name.trim() });
      onCreated(created);
      onSelect(created.id);
      setName("");
      setAdding(false);
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (result.items && result.items.length > 0) {
        ...
        setScanTotal(result.totalConfiable ? result.amount ?? null : null);
      } else {
      ...
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
        ...
        {tab === "ocr" ? (
          scanItems.length > 0 ? (
            <div className="space-y-3">
              ...
              {scanTotal !== null && Math.abs(sumaItemsElegidos() - scanTotal) > 1 && (
                <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 dark:bg-amber-500/10">
                  La suma de los ítems ({formatMoney(sumaItemsElegidos())}) no coincide con el total del ticket ({formatMoney(scanTotal)}).
                  Revisá los montos antes de guardar.
                </p>
              )}
              ...
              <CategoryPicker tripId={tripId} categories={categories} categoryId={categoryId} onSelect={setCategoryId} onCreated={handleCategoryCreated} />
          ...
          <div className="space-y-3">
            ...
            <CategoryPicker tripId={tripId} categories={categories} categoryId={categoryId} onSelect={setCategoryId} onCreated={handleCategoryCreated} />
            <PayersPicker members={trip.members} payers={payers} totalAmount={Number(amount) || 0} onChange={setPayers} />
            <div>
              <label className="text-xs font-medium text-slate-500 dark:text-slate-400">¿Entre quiénes se divide?</label>
              <div className="mt-2 flex flex-wrap gap-2">
                {trip.members.map((m) => {

...
import { AVATAR_COLOR_KEYS, AvatarColorKey, colorDeAvatar, iniciales } from "../Utilidades/formato";

interface EditProfileModalProps {
  onClose: () => void;
}

export function ModalEditarPerfil({ onClose }: EditProfileModalProps) {
  const { user, updateProfile } = useSesion();
  ...
  const previewColor = color ? colorDeAvatar(user.id, color) : colorDeAvatar(user.id);
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          {AVATAR_COLOR_KEYS.map((key) => {
            const c = colorDeAvatar(user.id, key);

...
import { Categoria, Proyecto } from "../Utilidades/tipos";

interface Props {
  trip: Proyecto;
  onClose: () => void;
  onUpdated: (trip: Proyecto) => void;
  onDeleted?: () => void;
}

export function ModalEditarProyecto({ trip, onClose, onUpdated, onDeleted }: Props) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const updated = await api.actualizarProyecto(trip.id, { name, startDate, endDate });
      onUpdated(updated);
      onClose();
    } catch (e: any) {
      setError(e.message);
  ...
  async function handleDeleteCategory(id: string) {
    ...
    try {
      await api.eliminarCategoria(trip.id, id);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.eliminarProyecto(trip.id);
      onDeleted();
    } catch (e: any) {
      setError(e.message);
      setDeleting(false);

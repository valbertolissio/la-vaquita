...
import { nombreVisible, formatearFecha, formatearPlata, resumenDePagadores } from "../Utilidades/formato";
...
export function ModalDetalleDeSaldo({ tripId, userId, onClose }: BalanceDetailModalProps) {
  ...
  const expenseRows: BreakdownRow[] = (expenses ?? [])
    .map((expense) => {
      ...
      return {
        ...
        subtitle: `${formatearFecha(expense.expenseDate)} · Pagó: ${resumenDePagadores(expense.payers, userId)}`,
        amount: paid - owed,
      };
    })
  ...
  async function undoPayment(payment: Pago) {
    ...
    try {
      await api.eliminarPago(tripId, payment.id);
      reload();
    } catch (e: any) {
      alert(e.message);
    } finally {

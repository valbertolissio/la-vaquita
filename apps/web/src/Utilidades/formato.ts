export function formatearPlata(amount: number, currency = "ARS") {
...
export function formatearFecha(date: string | Date) {
  return new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "short" }).format(new Date(date));
}

export function formatearDuracion(totalSeconds: number) {
 ...
 */
export function colorDeAvatar(userId: string, override?: string | null) {
...
export function nombreVisible(user: { name: string; nickname?: string | null }) {
...
export function resumenDePagadores(
  payers: { userId: string; user: { name: string; nickname?: string | null } }[],
  currentUserId?: string
) {
  const names = payers.map((p) => (p.userId === currentUserId ? "vos" : nombreVisible(p.user)));
